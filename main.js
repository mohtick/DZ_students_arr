...
let addStud = (event) => {
    ...
    let secndNameVal = secndName.value;
    let dateOfBirthVal = dateOfBirth.value;
    let facultyVal = faculty.value;


    if (firstNameVal === '' || secndNameVal === '' || dateOfBirthVal === '' || facultyVal === '') {
        let warngMsg = 'Error! All fields must be filled in!';
        warning.innerHTML = warngMsg;
        return;
    }
    warning.innerHTML = '';
    student.push(firstNameVal, secndNameVal, dateOfBirthVal, facultyVal);
